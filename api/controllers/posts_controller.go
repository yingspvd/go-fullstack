...
import (
	...
	"github.com/yingspvd/go-fullstack/api/auth"
	"github.com/yingspvd/go-fullstack/api/models"
	"github.com/yingspvd/go-fullstack/api/responses"
	"github.com/yingspvd/go-fullstack/api/utils/formaterror"
)
...
func (server *Server) UpdatePost(w http.ResponseWriter, r *http.Request) {
	...
	// Also chek if the request user id is equal to the one gotten from token
...
func (server *Server) DeletePost(w http.ResponseWriter, r *http.Request) {
	...
	if uid != uint64(post.AuthorID) {
		responses.ERROR(w, http.StatusUnauthorized, errors.New("Unauthorized"))
		return
	}

	_, err = post.DeleteAPost(server.DB, pid, uint32(uid))
	if err != nil {
		responses.ERROR(w, http.StatusBadRequest, err)
		return
	}
